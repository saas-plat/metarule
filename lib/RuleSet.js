...
module.exports = class RuleSet {
  ...
  constructor(name, sources = [], define, scope = {}) {
    ...
    try {
      ...
      this.flow = nools.getFlow(name) || nools.flow(name, (flow) => {
        for (let {
            name,
            when,
            then,
          ...
          // 兼容一下有可能简化写错的形式
          if (!when.every(it => Array.isArray(it))) {
            when = [when];
          }
    ...
    debug('%s load %s rules', this.name, sources.length);
  }
